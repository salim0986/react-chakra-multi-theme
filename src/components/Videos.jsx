...
const Videos = () => {
  ...
  return (
    <Stack minH={"100vh"} direction={["column", "column", "row"]}>
      <VStack w={"full"} p={5} gap={10}>
        <video
          controls
          controlsList="nodownload"
          src={videosArr[current]}
          style={{ width: "100%" }}
        ></video>
        <VStack w={"full"} py={15} alignItems={"flex-start"}>
          <Heading>Sample Video {current + 1}</Heading>
          <Text>
            This is a sample video description for video {current + 1} to check
            layout. Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Autem repellendus animi maiores facere explicabo. Magni omnis ut at,
            laborum nam eum saepe harum ab! Obcaecati quidem voluptates
            similique eaque natus!
          </Text>
        </VStack>
      </VStack>
      <VStack px={5} py={10} w={["full", "full", "xl"]} h={"full"} gap={"2rem"}>
        {videosArr.map((source, index) => {
          return (
            <Button
              key={index}
